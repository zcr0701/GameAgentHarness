...
- [x] 每个宏观方块使用8×8、共64个稀疏细粒子；未变化宏观格保持压缩状态。
- [x] 加入岩石、沙、水、岩浆数值材料表，以及粒子受损、流动/附着和细 Tilemap 投影。
- [x] 加入限时黑洞，验证地形/基岩/粒子/生物吞噬、刷怪门暂停和完整状态快照撤销。
- [x] 为 Harness 试炼样本添加岩石、沙、水、岩浆、基岩、粒子簇和生物填充原语；只写入镜头外隐藏房。
- [x] `-blackHolePhysicsSmokeTest`：填充样本、Sandbox 预演/实机提交、验证64粒子/块、全图吸附和精确撤销通过。
- [ ] 手动检查实际游戏中的细粒子显示、玩家碰撞、岩浆贴壁、水/沙流动和黑洞画面；当前 smoke 是无图形模式，不能代替视觉/手感验收。
- [ ] 采集目标机器的大地图帧时间、细粒子峰值展开量、TilemapCollider 刷新、粒子 VFX 采样和内存基线。
- [ ] 为普通材料填充、测试生物和其他副作用原语补通用可撤销 journal；当前已实现黑洞专属完整 snapshot 撤销。
