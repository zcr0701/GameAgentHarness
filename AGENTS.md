...
- **源码导读长期规则**：每次新增、删除、改名或跨模块调整源码时，同步维护 `docs/源码解读.md` 中的文件职责、关键入口、状态所有者和调用链；架构/行为细节同时更新对应模块设计文档。导读需明确已实现与计划的差别、验证命令和限制，面向未读过仓库的开发者。
...
| `GridTerrain.cs`、`MaterialParticleCatalog.cs`、`BlackHoleSpell.cs`、黑洞 Harness 接口 | 主协调者／gpt-6-sol；`docs/基础粒子.md` 初稿由 gpt-6-luna 完成 | 已集成；Windows 构建、有限物理 smoke、无限真实模型 smoke、地图爆破 smoke 通过 | 宏观格8×8=64粒子、来源可追溯的现实密度/强度参考、材料硬度损伤、基底玄武岩代理、黑洞限时/无限半径、有限范围与全图两种验收、粒子/生物吸入和 snapshot undo 已接线。`-blackHolePhysicsSmokeTest` 验证 Rock/基底阈值、五种材料局部吸入与精确撤销；真实 Q4_K_M `-creationBlackHoleSmokeTest` 41.87秒内吞噬16,067格、1,028,288个逻辑粒子、5,740格基底与4只敌人并通过；`-mapDemolitionSmokeTest` 验证72次爆破清除10,327个可破坏格并保留5,740格基底。物性和游戏标度分别见 `docs/现代物理学.md`、`docs/基础粒子.md`。视觉手感与目标机性能仍待人工测量 |
...
| 日期 | 观察 | 下次分派调整 |
| --- | --- | --- |
| 2026-09-25 | 用户将粒度定为每宏观方块8×8、共64粒子。Luna 在参数冻结后完成独立材料文档；Sol 负责稀疏地形状态、材料流动、黑洞吸附、敌人快照、Sandbox 候选与实机验收等跨系统代码。初稿保留了“尚未实现”的历史段落，集成时据当前源码更新。普通关卡 smoke 最初因岩石新增硬度8而继续使用旧的1点直接挖掘期望失败；修正为使用材料破坏阈值，并令 API 将未达阈值的伤害累积报告为已接受。最终 Unity Windows 构建无 C# warning/error；黑洞物理 smoke 验证8次1点伤害恰好移除1个粒子、全图16,073格/1,028,671逻辑粒子和5只生物吸入及精确 undo；隐藏房 Sandbox smoke 通过。 | 单文件材料参数表适合在数值/字段含义冻结后交 Luna；地形权威状态、Tilemap碰撞、物理预算和跨世界撤销由 Sol 设计集成。Luna 文档交付后仍由 Sol 对照源码更新已实现/待办并查验数值；粒子视觉及真实帧耗时不由无图形 smoke 推断。 |
...
| 2026-09-25 | 现代物理资料核对发现旧粒子页把部分游戏调参写成实验物性，Bedrock `(64, 3200)` 也没有材料来源；黑洞有限版预演错误地将未知局部命中投影成全图清空。Sol 新建 `现代物理学.md`，按 NIST、USGS、FHWA、Stanford、USDA 与岩芯实验来源记录试样条件，更新密度/强度标度；基底改用指定玄武岩数据代理，普通爆炸按阈值处理，地图清理通过 `PreserveBedrock` 按操作目标保留关卡边界。沙盒有限版改为要求实时范围复核，物理 smoke 新增基底阈值与有限范围/撤销断言。Windows 构建、`-blackHolePhysicsSmokeTest`、真实 Q4_K_M 无限放逐和 `-mapDemolitionSmokeTest` 均通过；模型依次选择 create/duration，41.87秒内吞噬16,067格、1,028,288粒子、5,740格基底和4只敌人，回执报告剩余敌人0。 | 材料表/研究文档可以在来源、试验条件和字段含义明确时由 Luna 单独起草；材料伤害、地形写入和黑洞验收同时影响多个状态，因此由 Sol 设计集成并逐条对照运行证据。文档不得把真实试样数值直接称为游戏伤害或宣称对爆炸波做了物理模拟。 |
...
| 2026-09-25 | 用户指出《现代物理学.md》新增了现实材料数据，却没有醒目呈现 Noita 粒子物理属性研究。将 Noita 公开材料字段族、职责和证据边界加入该文档，并与《基础粒子.md》的详细字段调研互链；官方来源只支持整体模拟架构，字段解释明确标为公开构建数据/社区模组资料，不冒充引擎源码结论。 | 同主题多份设计文档应设置清晰入口并标出证据来源层级；现实实验属性、Noita 游戏参数、本项目离散求解器参数分开记录。 |
