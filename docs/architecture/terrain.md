...
`GridTerrain` 是地形格子的唯一权威所有者。其私有 `Material[,] cells` 存放每个单元的材料状态：`Air`、`Rock`、`Sand`、`Water`、`Magma`、`Bedrock`。稀疏 `fineBlocks` 保存已展开宏观格中的细粒子占据掩码。数组、材料枚举和底层 `SetCell` 都是私有成员；其他脚本没有也不应获得直接读写数组的途径。

宏观 `Tilemap` 与单独的细粒子 `Tilemap` 只负责把权威材料/掩码投影为可见瓦片和 2D 碰撞形状。二者都由 `GridTerrain` 更新。外部模块若绕过 `GridTerrain` 直接调用 Tilemap 写瓦片，会制造“屏幕/碰撞显示为空、逻辑仍是实体”或反向不一致。

## 8×8 细粒子与材料表

`CellSize=0.5`，`FineParticlesPerSide=8`、`FineParticlesPerBlock=64`、`FineCellSize=0.0625`。宏观格材料数组仍是权威状态；完整块按单个材料字节压缩保存，只有受损/部分变化块在 `fineBlocks` 中展开为64位 `ulong Occupied` 掩码。细粒子使用名为 `Fine Particle Grid` 的稀疏 Grid/Tilemap 表现，不按粒子创建 Unity 对象或刚体。细 Tilemap 同时投影颗粒可见性和碰撞；水/岩浆细粒子 Tile 使用无碰撞瓦片。

材料属性集中在 `MaterialParticleCatalog`，顺序为硬度 HP/粒、密度 kg/m³、离散内聚阈值、粘度字段、离散壁面附着阈值：Rock `(8,2640,120,0.05,1)`、Sand `(3,1438,0.08,0.002,0.02)`、Water `(0.1,998.2043,0.01,0.001002,0.04)`、Magma `(0.5,2700,0.04,120,8)`、Bedrock/玄武岩代理 `(9,2892,240,0.05,1)`。Obsidian 与 Wood profile 分别为 `(16,2370,180,0.05,1.2)`、`(1,160,25,0.08,0.3)`。注明的密度来自对应试样或砂土单位重；水与岩浆粘度使用流体参考值，其他材料的粘度字段是求解器控制量。四种有抗压强度依据的固体 HP 由来源应力除以25MPa并向上取整，这是游戏映射而非真实爆压阈值；原型、条件、来源及局限见[现代物理与材料参数参考](../现代物理学.md)。细粒子按离散邻接规则应用硬度、密度、内聚、粘度和壁面附着；还没有连续材料力场、液体压力或浮力解算。普通伤害依各材质 HP，地图爆破原语显式绕开基底，黑洞绕开硬度并吞噬基底。

稀疏模拟上限为同时256个展开宏观格、每个更新 tick 检查512个细粒子。Rock/Sand/Water/Magma 另维护材料格索引；流体宏观格按固定 tick 走简化移动逻辑。爆破仍使用独立的候选扫描/批量 Tilemap 写入预算。数值与边界详见[基础粒子设计](../基础粒子.md)。
...
生成器写入底层起伏岩层、地图边界基底、五个房间基准地板、四段上升阶梯、固定种子平台、五处可挖岩门和各层沙堆。五层地板行是井口 `FloorCell(x)`、32、56、80、104；阶梯平台宽 5 格，中心左右各 2 格，末端与上层房间地板重叠接合。路线平台/沙堆/岩门使用固定随机种子 `84173` 或固定格子坐标，令相同版本和配置可重复生成，便于测试复现和人工定位。`SetInitial()` 限制初始结构写入范围。`TileFor()` 按材料返回对应瓦片；岩石/沙子的外观变化由坐标确定，因而同格重绘不产生随机材质跳变。`Air` 对应 `null` tile；基底采用玄武岩试样作为物性代理，常规高伤害可按其材料阈值破坏。地图爆破是有意保留结构边界的专用操作，会通过 `PulseSpec.PreserveBedrock` 跳过该材料。
...
| `DamageCircle(Vector2 center, float radius, int damage = 1)` | public | 将圆形范围映射到候选格子；对圆内格子尝试挖除或累积细粒子伤害，包含玄武岩代理基底。 | 仅正半径、正 damage 有效；若至少一格已记录伤害或颗粒已破坏则返回 true，次阈值伤害不会立即改变碰撞投影。 |
| `WorldToCell(Vector2)` / `CellCenter(int,int)` | internal | 世界坐标与格子索引/中心互转，供同程序集算法使用。 | `WorldToCell` 不钳制范围；调用者必须检查边界。 |
| `TryDamageCell(int,int)` | internal | 按材料硬度移除 Rock/Sand 或对已展开块施加细粒子伤害，更新数据与投影。 | 超界、Air、Bedrock 的普通伤害按规则拒绝；黑洞可专门提取 Bedrock。 |
...
| `FillBlocks` / `FillParticle` | internal | 受边界保护地填充宏观材料格或细粒子簇；Harness 只将它们定位到镜头外神之试炼房。 | 统一经 `GridTerrain` 写入；宏观格仍受材料索引维护。 |
| `ExtractCellParticles` | internal | 将一个宏观格的完整64粒子或稀疏剩余粒子移出地形并返回材料/粒子中心。 | BlackHoleSpell 批量调用，Bedrock 也可被黑洞移除。 |
| `CaptureSnapshot` / `RestoreSnapshot` | internal | 拷贝/恢复宏观材料数组与稀疏细粒子状态，并重建索引、Tilemap 和物理同步。 | 黑洞使用完整地形快照撤销；不是持久化或通用 journal。 |
...
玩法系统确定效果中心/线段
  ...
  → 地形检查边界及材料是否可挖
  → 私有写入路径更新 cells[x,y]、材料索引与细粒子掩码
  → 单格操作更新宏观 Tilemap 和必要细粒子 Tilemap；爆炸批次用 Tilemap.SetTiles
...
- `FixedUpdate()` 每累计约 0.10 秒更新沙、水和岩浆材料格；格索引分别由 `sandCellIndices`、`waterCellIndices`、`magmaCellIndices` 维护，不扫描整张地图。单轮以稳定索引顺序和复用缓冲执行简化直落/斜落流动。细粒子逻辑更新另受512次检查预算和256个展开格上限约束；宏观材料、细粒子掩码、两张 Tilemap 与材料索引必须由同一地形写入口同步。
...
- 游戏结束时材料更新停止。当前支持岩石/沙/水/岩浆细粒子表示与黑洞吸附，但没有气体、真实流体力学、目标硬件性能基线或网络复制层。

## 扩展约束

1. 新增可破坏材料时，明确它是否可被 `TryDamageCell` 移除、如何映射宏观/细粒子 Tile、是否参与重力/扩散、如何与碰撞同步；材料规则集中在 `GridTerrain` 与 `MaterialParticleCatalog`。
2. 修改材料伤害时，保持宏观格权威状态、64位粒子占据、损伤累积与两张 Tilemap 同步，不得让投射物私自维护影子耐久表。
...
- 地图仍保留一份 `Material[Width, Height]` 作为宏观逻辑权威；可见瓦片、TilemapCollider2D、材料索引和稀疏细粒子掩码都由 `GridTerrain` 同步维护，不可各自决定材料结果。
- 材料索引一致性不变量：沙、水、岩浆索引恰好包含相应材料的 row-major 索引 `y * Width + x`。初始化后重建；之后材料变化必须同步增删对应索引。不要从外部直接改数组、掩码或集合。
...
- 使用 `-blackHolePhysicsSmokeTest` 检查8×8粒子数量、岩石/沙/水/岩浆/基岩填充、直线吸附、实体吞噬、快照撤销与清理。
- 同一 smoke 还验证岩石7次1点伤害保留64粒子，第8次累计到 hardness=8 后只清除1个细粒子；普通关卡 smoke 用材料硬度阈值验证宏观通行和碰撞刷新。
